...
void CefBrowserHostImpl::RequestMediaAccessPermission(
  ...
  if (request.video_type == content::MEDIA_DISPLAY_VIDEO_CAPTURE){
        ...
        if (!picker) {
            std::move(callback).Run(content::MediaStreamDevices(), content::MEDIA_DEVICE_INVALID_STATE, nullptr);
        }
...
void CefBrowserHostImpl::OnPickerDialogResults(
    ...
    if (media_id.is_null()) {
        ...
        devices.push_back(content::MediaStreamDevice(
                                                     content::MEDIA_DISPLAY_VIDEO_CAPTURE, media_id.ToString(), media_id.ToString()));
    }
