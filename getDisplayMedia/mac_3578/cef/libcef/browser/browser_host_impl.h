...
class CefBrowserHostImpl : public CefBrowserHost,
 ...
 private:
  ...
  using RequestsQueue = base::circular_deque<std::unique_ptr<PendingAccessRequest>>;
  using RequestsQueues = base::flat_map<content::WebContents*, RequestsQueue>;
  RequestsQueues pending_requests_;
  std::unique_ptr<Cef::DesktopMediaPickerFactory> picker_factory_;
  void ProcessQueuedAccessRequest(const RequestsQueue& queue,content::WebContents* web_contents);
  void OnPickerDialogResults(content::WebContents* web_contents,content::DesktopMediaID source);
  //@zsf-e
