...
#import "libcef/browser/ui/cocoa/media_picker/desktop_media_picker_item.h"
...
@interface CefDesktopMediaPickerController ()

// Populate the window with controls and views.
- (void)initializeContentsWithParams:(const Cef::DesktopMediaPicker::Params&)params;
...
@implementation CefDesktopMediaPickerController

- (id)initWithSourceLists:
          (std::vector<std::unique_ptr<Cef::DesktopMediaList>>)sourceLists
                 callback:(const Cef::DesktopMediaPicker::DoneCallback&)callback
                   params:(const Cef::DesktopMediaPicker::Params&)params {
  ...
  if ((self = [super initWithWindow:window])) {
    ...
    bridge_.reset(new Cef::DesktopMediaPickerBridge(self));
  }
  return self;
}
...
- (void)sharePressed:(id)sender {
  ...
  CefDesktopMediaPickerItem* item = [items objectAtIndex:selectedIndex];
...
- (id)imageBrowser:(IKImageBrowserView*)browser itemAtIndex:(NSUInteger)index {
  DesktopMediaID::Type sourceType = [self sourceTypeForBrowser:browser];
  NSMutableArray* items = [self itemSetForType:sourceType];
  CefDesktopMediaPickerItem* item = [items objectAtIndex:index];
...
- (void)imageBrowser:(IKImageBrowserView*)browser
  ...
  CefDesktopMediaPickerItem* item;
...
#pragma mark CefDesktopMediaPickerObserver
...
- (void)sourceAddedForList:(Cef::DesktopMediaList*)list atIndex:(int)index {
  ...
  if (selectedIndex >= index)
    ...
    const Cef::DesktopMediaList::Source& source = list->GetSource(index);
  NSString* imageTitle = base::SysUTF16ToNSString(source.name);
  base::scoped_nsobject<CefDesktopMediaPickerItem> item(
      [[CefDesktopMediaPickerItem alloc] initWithSourceId:source.id
...
@end  // @interface CefDesktopMediaPickerController
