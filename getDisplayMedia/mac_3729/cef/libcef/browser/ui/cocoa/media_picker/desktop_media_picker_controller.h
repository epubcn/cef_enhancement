...
#import "libcef/browser/ui/cocoa/media_picker/desktop_media_picker_bridge.h"
...
@interface CefDesktopMediaPickerController
 ...
 @private
  ...
  std::vector<std::unique_ptr<Cef::DesktopMediaList>> sourceLists_;

  // To be called with the user selection.
  Cef::DesktopMediaPicker::DoneCallback doneCallback_;
 ...
 std::unique_ptr<Cef::DesktopMediaPickerBridge> bridge_;

  // Used to create |DesktopMediaPickerItem|s with unique IDs.
  int lastImageUID_;

  // Save input |params| for future use.
  Cef::DesktopMediaPicker::Params params_;
}
